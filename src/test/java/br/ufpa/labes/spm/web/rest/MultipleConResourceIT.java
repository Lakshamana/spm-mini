...
public class MultipleConResourceIT {
  ...
  public void setup() {
    ...
    this.restMultipleConMockMvc =
        MockMvcBuilders.standaloneSetup(multipleConResource)
  ...
  public static MultipleCon createEntity(EntityManager em) {
    MultipleCon multipleCon = new MultipleCon().fired(DEFAULT_FIRED);
    return multipleCon;
  }
  /**
  ...
  public static MultipleCon createUpdatedEntity(EntityManager em) {
    MultipleCon multipleCon = new MultipleCon().fired(UPDATED_FIRED);
    return multipleCon;
  }
  ...
  public void createMultipleCon() throws Exception {
    ...
    restMultipleConMockMvc
        .perform(
            post("/api/multiple-cons")
  ...
  public void getAllMultipleCons() throws Exception {
    ...
    restMultipleConMockMvc
        .perform(get("/api/multiple-cons?sort=id,desc"))
  ...
  public void getMultipleCon() throws Exception {
    ...
    // Get the multipleCon
    restMultipleConMockMvc
        .perform(get("/api/multiple-cons/{id}", multipleCon.getId()))
        .andExpect(status().isOk())
        .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
  ...
  public void getNonExistingMultipleCon() throws Exception {
    // Get the multipleCon
    restMultipleConMockMvc
        .perform(get("/api/multiple-cons/{id}", Long.MAX_VALUE))
  ...
  public void updateMultipleCon() throws Exception {
    ...
    // Disconnect from session so that the updates on updatedMultipleCon are not directly saved in
    // db
    em.detach(updatedMultipleCon);
    updatedMultipleCon.fired(UPDATED_FIRED);

    restMultipleConMockMvc
        .perform(
            put("/api/multiple-cons")
                .contentType(TestUtil.APPLICATION_JSON_UTF8)
  ...
  public void deleteMultipleCon() throws Exception {
    ...
    restMultipleConMockMvc
        .perform(
            delete("/api/multiple-cons/{id}", multipleCon.getId())
