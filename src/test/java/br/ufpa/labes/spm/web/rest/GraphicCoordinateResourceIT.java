...
public class GraphicCoordinateResourceIT {
  ...
  @Autowired private GraphicCoordinateRepository graphicCoordinateRepository;
  ...
  public void setup() {
    ...
    final GraphicCoordinateResource graphicCoordinateResource =
        new GraphicCoordinateResource(graphicCoordinateRepository);
    this.restGraphicCoordinateMockMvc =
        MockMvcBuilders.standaloneSetup(graphicCoordinateResource)
  ...
  public static GraphicCoordinate createEntity(EntityManager em) {
    GraphicCoordinate graphicCoordinate =
  ...
  public void createGraphicCoordinate() throws Exception {
    ...
    restGraphicCoordinateMockMvc
        .perform(
            post("/api/graphic-coordinates")
    ...
    GraphicCoordinate testGraphicCoordinate =
        graphicCoordinateList.get(graphicCoordinateList.size() - 1);
  ...
  public void getAllGraphicCoordinates() throws Exception {
    ...
    restGraphicCoordinateMockMvc
        .perform(get("/api/graphic-coordinates?sort=id,desc"))
        ...
        .andExpect(jsonPath("$.[*].theProcess").value(hasItem(DEFAULT_THE_PROCESS)));
  }
  ...
  public void getGraphicCoordinate() throws Exception {
    ...
    restGraphicCoordinateMockMvc
        .perform(get("/api/graphic-coordinates/{id}", graphicCoordinate.getId()))
        .andExpect(status().isOk())
        .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
        ...
        .andExpect(jsonPath("$.theProcess").value(DEFAULT_THE_PROCESS));
  }
  ...
  public void getNonExistingGraphicCoordinate() throws Exception {
    // Get the graphicCoordinate
    restGraphicCoordinateMockMvc
        .perform(get("/api/graphic-coordinates/{id}", Long.MAX_VALUE))
  ...
  public void updateGraphicCoordinate() throws Exception {
    ...
    GraphicCoordinate updatedGraphicCoordinate =
        graphicCoordinateRepository.findById(graphicCoordinate.getId()).get();
    // Disconnect from session so that the updates on updatedGraphicCoordinate are not directly
    // saved in db
  ...
  public void deleteGraphicCoordinate() throws Exception {
    ...
    restGraphicCoordinateMockMvc
        .perform(
            delete("/api/graphic-coordinates/{id}", graphicCoordinate.getId())
