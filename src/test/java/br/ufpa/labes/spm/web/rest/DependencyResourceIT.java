...
public class DependencyResourceIT {
  ...
  public void setup() {
    ...
    this.restDependencyMockMvc =
        MockMvcBuilders.standaloneSetup(dependencyResource)
  ...
  public static Dependency createEntity(EntityManager em) {
    Dependency dependency = new Dependency().kindDep(DEFAULT_KIND_DEP);
    return dependency;
  }
  /**
  ...
  public static Dependency createUpdatedEntity(EntityManager em) {
    Dependency dependency = new Dependency().kindDep(UPDATED_KIND_DEP);
    return dependency;
  }
  ...
  public void getAllDependencies() throws Exception {
    ...
    restDependencyMockMvc
        .perform(get("/api/dependencies?sort=id,desc"))
        .andExpect(status().isOk())
        .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
        .andExpect(jsonPath("$.[*].id").value(hasItem(dependency.getId().intValue())))
        .andExpect(jsonPath("$.[*].kindDep").value(hasItem(DEFAULT_KIND_DEP)));
  }
  ...
  public void getDependency() throws Exception {
    ...
    // Get the dependency
    restDependencyMockMvc
        .perform(get("/api/dependencies/{id}", dependency.getId()))
        .andExpect(status().isOk())
        .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
        .andExpect(jsonPath("$.id").value(dependency.getId().intValue()))
        .andExpect(jsonPath("$.kindDep").value(DEFAULT_KIND_DEP));
  }
  ...
  public void getNonExistingDependency() throws Exception {
    // Get the dependency
    restDependencyMockMvc
        .perform(get("/api/dependencies/{id}", Long.MAX_VALUE))
  ...
  public void updateDependency() throws Exception {
    ...
    updatedDependency.kindDep(UPDATED_KIND_DEP);
  ...
  public void deleteDependency() throws Exception {
    ...
    restDependencyMockMvc
        .perform(
            delete("/api/dependencies/{id}", dependency.getId())
