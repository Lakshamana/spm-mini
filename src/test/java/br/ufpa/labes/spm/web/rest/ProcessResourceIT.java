...
public class ProcessResourceIT {
  ...
  public void setup() {
    ...
    this.restProcessMockMvc =
        MockMvcBuilders.standaloneSetup(processResource)
  ...
  public static Process createEntity(EntityManager em) {
    Process process = new Process().ident(DEFAULT_IDENT).pState(DEFAULT_P_STATE);
    return process;
  }
  /**
  ...
  public static Process createUpdatedEntity(EntityManager em) {
    Process process = new Process().ident(UPDATED_IDENT).pState(UPDATED_P_STATE);
    return process;
  }
  ...
  public void getAllProcesses() throws Exception {
    ...
    restProcessMockMvc
        .perform(get("/api/processes?sort=id,desc"))
        ...
        .andExpect(jsonPath("$.[*].pState").value(hasItem(DEFAULT_P_STATE)));
  }
  ...
  public void getNonExistingProcess() throws Exception {
    // Get the process
    restProcessMockMvc
        .perform(get("/api/processes/{id}", Long.MAX_VALUE))
  ...
  public void updateProcess() throws Exception {
    ...
    updatedProcess.ident(UPDATED_IDENT).pState(UPDATED_P_STATE);
  ...
  public void deleteProcess() throws Exception {
    ...
    restProcessMockMvc
        .perform(
            delete("/api/processes/{id}", process.getId()).accept(TestUtil.APPLICATION_JSON_UTF8))
