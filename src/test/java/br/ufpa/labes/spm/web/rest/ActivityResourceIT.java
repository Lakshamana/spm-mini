...
public class ActivityResourceIT {
  ...
  @Mock private ActivityRepository activityRepositoryMock;
  ...
  public void setup() {
    ...
    this.restActivityMockMvc =
        MockMvcBuilders.standaloneSetup(activityResource)
  ...
  public static Activity createEntity(EntityManager em) {
    Activity activity =
        new Activity().ident(DEFAULT_IDENT).name(DEFAULT_NAME).isVersion(DEFAULT_IS_VERSION);
    return activity;
  }
  /**
  ...
  public static Activity createUpdatedEntity(EntityManager em) {
    Activity activity =
        new Activity().ident(UPDATED_IDENT).name(UPDATED_NAME).isVersion(UPDATED_IS_VERSION);
    return activity;
  }
  ...
  @Test
  @Transactional
  public void checkIdentIsRequired() throws Exception {
    int databaseSizeBeforeTest = activityRepository.findAll().size();
    // set the field null
    activity.setIdent(null);
  ...
  public void getAllActivitiesWithEagerRelationshipsIsEnabled() throws Exception {
    ...
    when(activityRepositoryMock.findAllWithEagerRelationships(any()))
        .thenReturn(new PageImpl(new ArrayList<>()));

    MockMvc restActivityMockMvc =
        MockMvcBuilders.standaloneSetup(activityResource)
            .setCustomArgumentResolvers(pageableArgumentResolver)
            .setControllerAdvice(exceptionTranslator)
            .setConversionService(createFormattingConversionService())
    ...
    restActivityMockMvc.perform(get("/api/activities?eagerload=true")).andExpect(status().isOk());
  ...
  public void getNonExistingActivity() throws Exception {
    // Get the activity
    restActivityMockMvc
        .perform(get("/api/activities/{id}", Long.MAX_VALUE))
  ...
  public void updateActivity() throws Exception {
    ...
    updatedActivity.ident(UPDATED_IDENT).name(UPDATED_NAME).isVersion(UPDATED_IS_VERSION);
  ...
  public void deleteActivity() throws Exception {
    ...
    restActivityMockMvc
        .perform(
            delete("/api/activities/{id}", activity.getId()).accept(TestUtil.APPLICATION_JSON_UTF8))
