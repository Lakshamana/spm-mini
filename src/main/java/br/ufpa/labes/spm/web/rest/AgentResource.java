...
public class AgentResource {
   ...
   *     agent, or with status {@code 400 (Bad Request)} if the agent has already an ID.
  ...
  public ResponseEntity<Agent> createAgent(@Valid @RequestBody Agent agent)
    ...
    if (agent.getId() != null) {
      throw new BadRequestAlertException(
          "A new agent cannot already have an ID", ENTITY_NAME, "idexists");
    }
   ...
   *     agent, or with status {@code 400 (Bad Request)} if the agent is not valid, or with status
    ...
    return ResponseEntity.ok()
        .headers(
            HeaderUtil.createEntityUpdateAlert(
                applicationName, true, ENTITY_NAME, agent.getId().toString()))
        .body(result);
  }
   ...
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the agent, or
