...
public class MultipleConResource {
   ...
   *     multipleCon, or with status {@code 400 (Bad Request)} if the multipleCon has already an ID.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PostMapping("/multiple-cons")
  public ResponseEntity<MultipleCon> createMultipleCon(@RequestBody MultipleCon multipleCon)
   ...
   *     multipleCon, or with status {@code 400 (Bad Request)} if the multipleCon is not valid, or
   *     with status {@code 500 (Internal Server Error)} if the multipleCon couldn't be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PutMapping("/multiple-cons")
  public ResponseEntity<MultipleCon> updateMultipleCon(@RequestBody MultipleCon multipleCon)
      throws URISyntaxException {
    ...
    return ResponseEntity.ok()
        .headers(
            HeaderUtil.createEntityUpdateAlert(
                applicationName, true, ENTITY_NAME, multipleCon.getId().toString()))
        .body(result);
  }
   ...
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the multipleCon,
