...
public class SequenceResource {
   ...
   *     sequence, or with status {@code 400 (Bad Request)} if the sequence has already an ID.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PostMapping("/sequences")
  public ResponseEntity<Sequence> createSequence(@RequestBody Sequence sequence)
   ...
   *     sequence, or with status {@code 400 (Bad Request)} if the sequence is not valid, or with
   *     status {@code 500 (Internal Server Error)} if the sequence couldn't be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
    ...
    return ResponseEntity.ok()
        .headers(
            HeaderUtil.createEntityUpdateAlert(
                applicationName, true, ENTITY_NAME, sequence.getId().toString()))
        .body(result);
  }
   ...
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the list of sequences in
   *     body.
   */
  ...
  public List<Sequence> getAllSequences(@RequestParam(required = false) String filter) {
    if ("thedependency-is-null".equals(filter)) {
      ...
      return StreamSupport.stream(sequenceRepository.findAll().spliterator(), false)
   ...
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the sequence, or
