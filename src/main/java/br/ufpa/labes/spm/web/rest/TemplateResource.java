...
public class TemplateResource {
   ...
   *     template, or with status {@code 400 (Bad Request)} if the template has already an ID.
  ...
  public ResponseEntity<Template> createTemplate(@RequestBody Template template)
    ...
    if (template.getId() != null) {
      throw new BadRequestAlertException(
          "A new template cannot already have an ID", ENTITY_NAME, "idexists");
    }
   ...
   *     template, or with status {@code 400 (Bad Request)} if the template is not valid, or with
   *     status {@code 500 (Internal Server Error)} if the template couldn't be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
    ...
    return ResponseEntity.ok()
        .headers(
            HeaderUtil.createEntityUpdateAlert(
                applicationName, true, ENTITY_NAME, template.getId().toString()))
        .body(result);
  }
   ...
   *
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the list of templates in
   *     body.
   */
   ...
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the template, or
