...
public class ConnectionResource {
   ...
   *     connection, or with status {@code 400 (Bad Request)} if the connection has already an ID.
  ...
  public ResponseEntity<Connection> createConnection(@Valid @RequestBody Connection connection)
    ...
    if (connection.getId() != null) {
      throw new BadRequestAlertException(
          "A new connection cannot already have an ID", ENTITY_NAME, "idexists");
    }
   ...
   *     connection, or with status {@code 400 (Bad Request)} if the connection is not valid, or
   *     with status {@code 500 (Internal Server Error)} if the connection couldn't be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PutMapping("/connections")
  public ResponseEntity<Connection> updateConnection(@Valid @RequestBody Connection connection)
      throws URISyntaxException {
    ...
    return ResponseEntity.ok()
        .headers(
            HeaderUtil.createEntityUpdateAlert(
                applicationName, true, ENTITY_NAME, connection.getId().toString()))
        .body(result);
  }
   ...
   *
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the list of connections in
   *     body.
   */
   ...
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the connection,
