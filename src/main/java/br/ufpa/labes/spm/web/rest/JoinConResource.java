...
public class JoinConResource {
   ...
   *     joinCon, or with status {@code 400 (Bad Request)} if the joinCon has already an ID.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PostMapping("/join-cons")
  public ResponseEntity<JoinCon> createJoinCon(@RequestBody JoinCon joinCon)
      throws URISyntaxException {
    log.debug("REST request to save JoinCon : {}", joinCon);
    if (joinCon.getId() != null) {
      throw new BadRequestAlertException(
          "A new joinCon cannot already have an ID", ENTITY_NAME, "idexists");
    }
   ...
   *     joinCon, or with status {@code 400 (Bad Request)} if the joinCon is not valid, or with
   *     status {@code 500 (Internal Server Error)} if the joinCon couldn't be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
   ...
   *     many-to-many).
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the list of joinCons in
   *     body.
   */
  @GetMapping("/join-cons")
  public List<JoinCon> getAllJoinCons(
   ...
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the joinCon, or
