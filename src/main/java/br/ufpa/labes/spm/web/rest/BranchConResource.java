...
public class BranchConResource {
   ...
   *     branchCon, or with status {@code 400 (Bad Request)} if the branchCon has already an ID.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PostMapping("/branch-cons")
  public ResponseEntity<BranchCon> createBranchCon(@RequestBody BranchCon branchCon)
   ...
   *     branchCon, or with status {@code 400 (Bad Request)} if the branchCon is not valid, or with
   *     status {@code 500 (Internal Server Error)} if the branchCon couldn't be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PutMapping("/branch-cons")
  public ResponseEntity<BranchCon> updateBranchCon(@RequestBody BranchCon branchCon)
      throws URISyntaxException {
    ...
    return ResponseEntity.ok()
        .headers(
            HeaderUtil.createEntityUpdateAlert(
                applicationName, true, ENTITY_NAME, branchCon.getId().toString()))
        .body(result);
  }
   ...
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the branchCon, or
