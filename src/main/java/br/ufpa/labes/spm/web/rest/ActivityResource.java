...
public class ActivityResource {
   ...
   *     activity, or with status {@code 400 (Bad Request)} if the activity has already an ID.
  ...
  public ResponseEntity<Activity> createActivity(@Valid @RequestBody Activity activity)
    ...
    if (activity.getId() != null) {
      throw new BadRequestAlertException(
          "A new activity cannot already have an ID", ENTITY_NAME, "idexists");
    }
   ...
   *     activity, or with status {@code 400 (Bad Request)} if the activity is not valid, or with
   *     status {@code 500 (Internal Server Error)} if the activity couldn't be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
    ...
    return ResponseEntity.ok()
        .headers(
            HeaderUtil.createEntityUpdateAlert(
                applicationName, true, ENTITY_NAME, activity.getId().toString()))
        .body(result);
  }
   ...
   *     many-to-many).
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the list of activities in
   *     body.
   */
   ...
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the activity, or
