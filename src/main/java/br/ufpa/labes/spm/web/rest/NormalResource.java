...
public class NormalResource {
   ...
   *     normal, or with status {@code 400 (Bad Request)} if the normal has already an ID.
   ...
   *     normal, or with status {@code 400 (Bad Request)} if the normal is not valid, or with status
   *     {@code 500 (Internal Server Error)} if the normal couldn't be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PutMapping("/normals")
    ...
    return ResponseEntity.ok()
        .headers(
            HeaderUtil.createEntityUpdateAlert(
                applicationName, true, ENTITY_NAME, normal.getId().toString()))
        .body(result);
  }
   ...
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the normal, or
