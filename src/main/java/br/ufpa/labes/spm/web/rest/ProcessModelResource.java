...
public class ProcessModelResource {
   ...
   *     processModel, or with status {@code 400 (Bad Request)} if the processModel has already an
   *     ID.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PostMapping("/process-models")
  public ResponseEntity<ProcessModel> createProcessModel(@RequestBody ProcessModel processModel)
      throws URISyntaxException {
    log.debug("REST request to save ProcessModel : {}", processModel);
    if (processModel.getId() != null) {
      throw new BadRequestAlertException(
          "A new processModel cannot already have an ID", ENTITY_NAME, "idexists");
    }
   ...
   *     processModel, or with status {@code 400 (Bad Request)} if the processModel is not valid, or
   *     with status {@code 500 (Internal Server Error)} if the processModel couldn't be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PutMapping("/process-models")
  public ResponseEntity<ProcessModel> updateProcessModel(@RequestBody ProcessModel processModel)
      throws URISyntaxException {
    ...
    return ResponseEntity.ok()
        .headers(
            HeaderUtil.createEntityUpdateAlert(
                applicationName, true, ENTITY_NAME, processModel.getId().toString()))
        .body(result);
  }
   ...
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and the list of processModels
   *     in body.
   */
  ...
  public List<ProcessModel> getAllProcessModels(@RequestParam(required = false) String filter) {
    if ("thedecomposed-is-null".equals(filter)) {
      ...
      return StreamSupport.stream(processModelRepository.findAll().spliterator(), false)
   ...
   * @return the {@link ResponseEntity} with status {@code 200 (OK)} and with body the processModel,
  ...
  public ResponseEntity<ProcessModel> getProcessModel(@PathVariable Long id) {
    ...
    Optional<ProcessModel> processModel = processModelRepository.findOneWithEagerRelationshipsById(id);
