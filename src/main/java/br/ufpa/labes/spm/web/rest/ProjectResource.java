...
/** REST controller for managing {@link br.ufpa.labes.spm.domain.Project}. */
@RestController
@RequestMapping("/api")
public class ProjectResource {
  ...
  public ResponseEntity<Project> createProject(@Valid @RequestBody Project project)
    ...
    if (project.getId() != null) {
      throw new BadRequestAlertException(
          "A new project cannot already have an ID", ENTITY_NAME, "idexists");
    }
   ...
   *     project, or with status {@code 400 (Bad Request)} if the project is not valid, or with
   *     status {@code 500 (Internal Server Error)} if the project couldn't be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PutMapping("/projects")
    ...
    return ResponseEntity.ok()
        .headers(
            HeaderUtil.createEntityUpdateAlert(
                applicationName, true, ENTITY_NAME, project.getId().toString()))
        .body(result);
  }
