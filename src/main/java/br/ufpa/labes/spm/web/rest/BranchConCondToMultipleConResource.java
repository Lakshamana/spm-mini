...
public class BranchConCondToMultipleConResource {
   ...
   *     branchConCondToMultipleCon, or with status {@code 400 (Bad Request)} if the
   *     branchConCondToMultipleCon has already an ID.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PostMapping("/branch-con-cond-to-multiple-cons")
  public ResponseEntity<BranchConCondToMultipleCon> createBranchConCondToMultipleCon(
      @RequestBody BranchConCondToMultipleCon branchConCondToMultipleCon)
    ...
    BranchConCondToMultipleCon result =
   ...
   *     branchConCondToMultipleCon, or with status {@code 400 (Bad Request)} if the
   *     branchConCondToMultipleCon is not valid, or with status {@code 500 (Internal Server Error)}
   *     if the branchConCondToMultipleCon couldn't be updated.
   * @throws URISyntaxException if the Location URI syntax is incorrect.
   */
  @PutMapping("/branch-con-cond-to-multiple-cons")
  public ResponseEntity<BranchConCondToMultipleCon> updateBranchConCondToMultipleCon(
      @RequestBody BranchConCondToMultipleCon branchConCondToMultipleCon)
      throws URISyntaxException {
    ...
    return ResponseEntity.ok()
        .headers(
            HeaderUtil.createEntityUpdateAlert(
                applicationName, true, ENTITY_NAME, branchConCondToMultipleCon.getId().toString()))
        .body(result);
  }
   ...
   *     branchConCondToMultipleCon, or with status {@code 404 (Not Found)}.
   */
  @GetMapping("/branch-con-cond-to-multiple-cons/{id}")
  public ResponseEntity<BranchConCondToMultipleCon> getBranchConCondToMultipleCon(
      @PathVariable Long id) {
    log.debug("REST request to get BranchConCondToMultipleCon : {}", id);
    Optional<BranchConCondToMultipleCon> branchConCondToMultipleCon =
        branchConCondToMultipleConRepository.findById(id);
