...
public class EasyModelingServicesImpl implements EasyModelingServices {
  ...
  @Autowired private ProcessRepository procRepository;

  // NotificationServices remote;

  @Autowired private DecomposedRepository decRepository;

  @Autowired private ActivityRepository activityRepository;

  @Autowired private NormalRepository normRepository;

  // @Autowired
  // private AutomaticRepository autoRepository;

  @Autowired private ArtifactRepository artRepository;

  // @Autowired
  // private InvolvedArtifactRepository involvedRepository;

  @Autowired private GraphicCoordinateRepository coordRepository;

  @Autowired private ProcessModelRepository pmodelRepository;
  ...
  @Autowired private ArtifactConRepository artConRepository;

  // @Autowired
  // private ArtifactTypeRepository artTypeRepository;

  @Autowired private MultipleConRepository multiRepository;

  @Autowired private ConnectionRepository conRepository;

  @Autowired private BranchConCondToMultipleConRepository bctmcRepository;

  @Autowired private JoinConRepository joinConRepository;

  @Autowired private BranchConRepository branchConRepository;
  ...
  @Autowired ReqAgentRepository reqAgentRepository;
  ...
  @Autowired ReqWorkGroupRepository reqWorkGroupRepository;
  ...
  @Autowired private WebAPSEEObjectRepository webAPSEEObjRepository;
  ...
  public String copyActivity(String act_id, String level_to_copy) {
    ...
    // 						Messages.getString("facades.DynamicModeling.DaoExcProcess") + process_id +
    // Messages.getString("facades.DynamicModeling.DaoExc_NotFound"));
    ...
    // 				try {
    // 					throw new
    // RepositoryQueryException(Messages.getString("facades.DynamicModeling.DaoExcDecomActv") +
    // currentModel //$NON-NLS-1$
    ...
    // 			try {
    // 				throw new RepositoryQueryException(
    // 						Messages.getString("facades.DynamicModeling.ModelingExcActv") + act_id +
    // Messages.getString("facades.DynamicModeling.DaoExcNotFound"));
    // 			} catch (RepositoryQueryException e) {
    // 				// TODO Auto-generated catch block
    // 				e.printStackTrace();
    ...
    // 				if (!(ProcessModelState.equals(ProcessModel.CANCELED) ||
    // ProcessModelState.equals(ProcessModel.FAILED) || ProcessModelState
    ...
    // 					try {
    // 						throw new ModelingException("Process Model not ready to receive copies. It has already
    // been concluded!");
    ...
    // 				// BaseRepository.closeSession(currentSession);
    // 				try {
    // 					throw new ModelingException("Process " + process_id + " not ready to receive copies. It
    ...
    // 			if (!(decomposedState.equals(ProcessModel.CANCELED) ||
    // decomposedState.equals(ProcessModel.FAILED) || decomposedState
    // 					.equals(ProcessModel.FINISHED))) {
  ...
  public void copyActivities(String[] act_ids, String[] cons_ids, String level_to_copy) {
    ...
    if (proc == null)
      try {
        throw new RepositoryQueryException(
            Messages.getString("facades.DynamicModeling.DaoExcProcess")
                + process_id
                + Messages.getString("facades.DynamicModeling.DaoExc_NotFound"));
      } catch (RepositoryQueryException e) {
        // TODO Auto-generated catch block
    ...
    if (st.hasMoreTokens()) {
      ...
      if (dec == null)
        try {
          throw new RepositoryQueryException(
              Messages.getString("facades.DynamicModeling.DaoExcDecomActv")
                  + currentModel //$NON-NLS-1$
                  + Messages.getString("facades.DynamicModeling.DaoExcNotFound"));
        } catch (RepositoryQueryException e) {
          // TODO Auto-generated catch block
    ...
    for (int i = 0; i < act_ids.length; i++) {
      ...
      if (acts[i] == null) {
        ...
        try {
          throw new RepositoryQueryException(
              Messages.getString("facades.DynamicModeling.ModelingExcActv")
                  + act_id
                  + Messages.getString("facades.DynamicModeling.DaoExcNotFound"));
        } catch (RepositoryQueryException e) {
          // TODO Auto-generated catch block
    ...
    if (processIdent.equals(level_to_copy)) {
      ...
      if (processState.equals(Process.NOT_STARTED) || processState.equals(Process.ENACTING)) {
        ...
        if (!(ProcessModelState.equals(ProcessModel.CANCELED)
            ...
            || ProcessModelState.equals(ProcessModel.FINISHED))) {
          ...
          try {
            throw new ModelingException(
                "Process Model not ready to receive copies. It has already been concluded!");
          } catch (ModelingException e) {
            // TODO Auto-generated catch block
            e.printStackTrace();
          }
        ...
        try {
          throw new ModelingException(
              "Process "
                  + process_id
                  + " not ready to receive copies. It has already been concluded!");
        } catch (ModelingException e) {
          // TODO Auto-generated catch block
          e.printStackTrace();
        }
      ...
      String decomposedState = pmodel.getPmState();
      if (!(decomposedState.equals(ProcessModel.CANCELED)
          || decomposedState.equals(ProcessModel.FAILED)
          || decomposedState.equals(ProcessModel.FINISHED))) {
        // TODO Adapt for new coordinates model
        // Collection<Activity> newActivities =
  ...
  // public void calculateUtilsDays(long new_begin_date, long new_end_date,CalendarDTO
  // replainDates){
  ...
  // 	System.out.print("inicio" +plannedBegin+"
  // "+plannedEnd+"array"+replainDates.getNotWorkingDays().get(0));
  ...
  //           //começa o paranauê
  //           if (plannedBegin.after(notWorkingDays.get(i)) ||
  // plannedEnd.before(notWorkingDays.get(i))) {
  //           System.out.println("caiu no if"+notWorkingDays.get(i));
  //           newBeginDate = plannedBegin;
  //   		newEndDate = plannedEnd;
  //           }else if (plannedBegin.before(notWorkingDays.get(i)) &&
  // plannedEnd.after(notWorkingDays.get(i))){
  ...
  //                        inicial = inicial + 1;
  //                        //   begin = new Date(begin.getTime()+plannedBegin.getTime() +
  // MILLIS_PER_DAY);
  ...
  //           	 newEndDate =  end;
  // 			newBeginDate = plannedBegin;
  // 			System.out.println("nova data inicial parcial"+newBeginDate+" nniva data final
  // parcial"+newEndDate);
  ...
  // @Override
  // public void replanningDates(String act_id, long new_begin_date, long new_end_date, CalendarDTO
  // replanningDates) {
  ...
  // 	if (!(state.equals(Plain.FINISHED) || state.equals(Plain.FAILED) ||
  ...
  // 		//Chamar metodo que calcula as datas válidas de inicio e fim da atividade de acordo com o
  ...
  // 		try {
  // 			throw new
  // ModelingException(Messages.getString("facades.DynamicModeling.ReplanningFinishedActivity"));
  // 		} catch (ModelingException e) {
  // 			// TODO Auto-generated catch block
  // 			e.printStackTrace();
  // 		}
  ...
  private Activity cloneActivity(
      Activity activity, String level_to, Hashtable<String, String> coordinates)
      throws RepositoryQueryException {
    ...
    if (activity != null) {
      if (activity instanceof Normal) {
        ...
        this.addCoordinate(
            ...
            WebAPSEEObject.ACTIVITY + "+" + newIdent,
            coordinates);
      ...
      // String newIdent = this.generateNewIdent(activity.getIdent(), level_to, this.allActs);
      // clone.setIdent(newIdent);
      // this.addCoordinate(activity.getId(), activity.getClass().getSimpleName(),
      // WebAPSEEObject.ACTIVITY + "+" + newIdent, coordinates);
   ...
   * @throws RepositoryQueryException
   */
  private void copyNormalProperties(
      Normal destinationNormal, Normal sourceNormal, Hashtable<String, String> coordinates)
      throws RepositoryQueryException {
    // simple attributes
    ...
    // newInvolvedArtifactsToNormal =
    // copyInvolvedArtifacts(sourceNormal.getInvolvedArtifactToNormal(), destinationNormal,
    // TO_INVOLVED);
    ...
    // newInvolvedArtifactsFromNormal =
    // copyInvolvedArtifacts(sourceNormal.getInvolvedArtifactFromNormal(), destinationNormal,
    // FROM_INVOLVED);
    ...
    // newRequiredResources = copyRequiredResources(sourceNormal.getTheRequiredResource(),
    // destinationNormal, coordinates);
    ...
    // newRequiredPeople = copyRequiredPeople(sourceNormal.getTheRequiredPeople(),
    // destinationNormal, coordinates);
   ...
   */
  // private Collection<Reservation> copyReservations(Collection theReservation, Normal
  // destinationNormal) {
  // 	Collection<Reservation> newInvolvedReservations = new HashSet<Reservation>();
  // 	for (Iterator<Reservation> reservationsIterator = theReservation.iterator();
  ...
  private Collection<InvolvedArtifact> copyInvolvedArtifacts(
      Collection<InvolvedArtifact> currentInvolvedArtifacts,
      Normal newNormalReference,
      int kindRelationship) {

    Collection<InvolvedArtifact> newInvolvedArtifacts = new HashSet<InvolvedArtifact>();
    for (Iterator<InvolvedArtifact> involvedIterator = currentInvolvedArtifacts.iterator();
        involvedIterator.hasNext(); ) {
  ...
  // private Collection<RequiredResource> copyRequiredResources(Collection<RequiredResource>
  // theRequiredResource, Normal destinationNormal,
  ...
  // 	for (Iterator<RequiredResource> requiredIterator = theRequiredResource.iterator();
  // requiredIterator.hasNext();) {
  ...
  // 			this.addCoordinate(currentReqResource.getId(),
  // currentReqResource.getClass().getSimpleName(), WebAPSEEObject.REQ_RESOURCE + "+"
  ...
  private Collection<RequiredPeople> copyRequiredPeople(
      Collection theRequiredPeople, Normal destinationNormal, Hashtable<String, String> coordinates)
      throws RepositoryQueryException {
    Collection<RequiredPeople> newRequiredPeoples = new HashSet<RequiredPeople>();

    String coordinateKey = null;

    for (Iterator<RequiredPeople> requiredIterator = theRequiredPeople.iterator();
        requiredIterator.hasNext(); ) {
      ...
      if (currentReqPeople != null) {
        ...
        if (currentReqPeople instanceof ReqAgent) {
          ...
          // 	((ReqAgent) newRequiredPeople).insertIntoTheRole(((ReqAgent)
          // currentReqPeople).getTheRole());
          ...
          if (((ReqAgent) currentReqPeople).getTheAgent() != null) {
            ...
            coordinateKey =
                coordinateKey + ":" + ((ReqAgent) newRequiredPeople).getTheAgent().getIdent();
          }
          // about ReqAgentRequiresAbility
          // Collection<ReqAgentRequiresAbility> newReqAgReqAbility = null;
          // newReqAgReqAbility = copyReqAgentReqAbility(((ReqAgent)
          // currentReqPeople).getTheReqAgentRequiresAbilities(),
          // 		((ReqAgent) newRequiredPeople));
          // ((ReqAgent)
          // newRequiredPeople).setTheReqAgentRequiresAbilities((Set<ReqAgentRequiresAbility>)
          // newReqAgReqAbility);

          coordinateKey = coordinateKey + ":" + destinationNormal.getIdent();
          this.addCoordinate(
              ((ReqAgent) currentReqPeople).getId(),
              ((ReqAgent) currentReqPeople).getClass().getSimpleName(),
              WebAPSEEObject.REQ_AGENT + "+" + coordinateKey,
              coordinates);
          coordinateKey = null;
          ...
          // 	((ReqWorkGroup) newRequiredPeople).setTheWorkGroupType(((ReqWorkGroup)
          // currentReqPeople).getTheWorkGroupType());
          ...
          if (((ReqWorkGroup) currentReqPeople).getTheWorkGroup() != null) {
            ((ReqWorkGroup) newRequiredPeople)
                .setTheWorkGroup(((ReqWorkGroup) currentReqPeople).getTheWorkGroup());
            coordinateKey =
                coordinateKey
                    + ":"
                    + ((ReqWorkGroup) newRequiredPeople).getTheWorkGroup().getIdent();
          }

          coordinateKey = coordinateKey + ":" + destinationNormal.getIdent();
          this.addCoordinate(
              ((ReqWorkGroup) currentReqPeople).getId(),
              ((ReqWorkGroup) currentReqPeople).getClass().getSimpleName(),
              WebAPSEEObject.REQ_GROUP + "+" + coordinateKey,
              coordinates);
          coordinateKey = null;
        } // end if
  ...
  // private Collection<ReqAgentRequiresAbility> copyReqAgentReqAbility(Collection
  // theReqAgentRequiresAbility, ReqAgent newReqAgent) {
  // 	Collection<ReqAgentRequiresAbility> newReqAgReqAbilities = new
  // HashSet<ReqAgentRequiresAbility>();

  // 	for (Iterator<ReqAgentRequiresAbility> requiredIterator =
  // theReqAgentRequiresAbility.iterator(); requiredIterator.hasNext();) {
  ...
  private void copyDecomposedProperties(
      Decomposed newDecomposed,
      Decomposed currentDecomposed,
      Hashtable<String, String> coordinates) {
    ...
    if (currentDecomposed.getTheReferedProcessModel() != null) {
      ...
      coordinates.putAll(
          copyProcessModelData(
              currentDecomposed.getTheReferedProcessModel(),
              newReferedProcessModel,
              newDecomposed.getIdent()));
      newDecomposed.setTheReferedProcessModel(newReferedProcessModel);
    }
  } // END METHOD

  @Override
  public Hashtable<String, String> copyProcessModelData(
      ProcessModel oldProcessModel, ProcessModel newProcessModel, String level_to) {
    try {
      ...
      newConnections =
          copyConnectionsOnProcessModel(connections, newActivities, level_to, coordinates);
  ...
  private Hashtable<String, Activity> copyActivitiesOnProcessModel(
      Collection<Activity> activities, String level_to, Hashtable<String, String> coordinates)
      throws RepositoryQueryException {
    Hashtable<String, Activity> newActivities = new Hashtable<String, Activity>();

    for (Iterator<Activity> activityIterator = activities.iterator();
      ...
      if (currentActivity != null) {
        ...
        this.addCoordinate(
            ...
            WebAPSEEObject.ACTIVITY + "+" + newActivityIdent,
            coordinates);
        ...
        // }
          ...
          newActivity.setIdent(newActivityIdent);
          copyDecomposedProperties(
              ((Decomposed) newActivity), ((Decomposed) currentActivity), coordinates);
        }
   ...
   * @throws RepositoryQueryException
   */
  private Set<Connection> copyConnectionsOnProcessModel(
    ...
    for (Iterator<Connection> connectionsIterator = connections.iterator();
        connectionsIterator.hasNext(); ) {
      ...
      if (currentConnection != null) {
        ...
        if (currentConnection instanceof Sequence) {
          ...
          if (((Sequence) currentConnection).getTheDependency() != null) {
            ...
            newDependency.setKindDep(
                ((Sequence) currentConnection).getTheDependency().getKindDep());
          ...
          if (((Sequence) currentConnection).getToActivity() != null) {
            // new activity ident
            Activity newToAct =
                activitiesTable.get(((Sequence) currentConnection).getToActivity().getIdent());
            if (newToAct != null) {
          ...
          if (((Sequence) currentConnection).getFromActivity() != null) {
            // new activity ident
            Activity newFromAct =
                activitiesTable.get(((Sequence) currentConnection).getFromActivity().getIdent());
            if (newFromAct != null) {
          ...
          this.addCoordinate(
              ((Sequence) currentConnection).getId(),
              ((Sequence) currentConnection).getClass().getSimpleName(),
              WebAPSEEObject.CONNECTION + "+" + newConnectionIdent,
              coordinates);
        } // ########################
          ...
          if (((Feedback) currentConnection).getToActivity() != null) {
            // new activity ident
            Activity newToAct =
                activitiesTable.get(((Feedback) currentConnection).getToActivity().getIdent());
            if (newToAct != null) {
          ...
          if (((Feedback) currentConnection).getFromActivity() != null) {
            // new activity ident
            Activity newFromAct =
                activitiesTable.get(((Feedback) currentConnection).getFromActivity().getIdent());
            if (newFromAct != null) {
          ...
          // 	((Feedback) newConnection).insertIntoTheCondition(((Feedback)
          // currentConnection).getTheCondition().createClone());
          // }// end if condition != null

          newConnection.setIdent(newConnectionIdent);
          this.addCoordinate(
              ((Feedback) currentConnection).getId(),
              ((Feedback) currentConnection).getClass().getSimpleName(),
              WebAPSEEObject.CONNECTION + "+" + newConnectionIdent,
              coordinates);
        } // ########################
          ...
          // 	((ArtifactCon) newConnection).setTheArtifactType(((ArtifactCon)
          // currentConnection).getTheArtifactType());
          // }
          ...
          for (Iterator<Activity> toActivityIterator = toActivities.iterator();
              toActivityIterator.hasNext(); ) {
          ...
          for (Iterator<Activity> fromActivityIterator = fromActivities.iterator();
              fromActivityIterator.hasNext(); ) {
          ...
          this.addCoordinate(
              ((ArtifactCon) currentConnection).getId(),
              ((ArtifactCon) currentConnection).getClass().getSimpleName(),
              WebAPSEEObject.CONNECTION + "+" + newConnectionIdent,
              coordinates);
        } // ########################
          ...
          if (currentConnection instanceof BranchANDCon) {
            ...
            ((BranchConCond) newConnection)
                .setKindBranch(((BranchConCond) currentConnection).getKindBranch());

            // ToActivity
            Collection toActivities =
                ((BranchConCond) currentConnection).getTheBranchConCondToActivities();
            for (Iterator<BranchConCondToActivity> toActivityIterator = toActivities.iterator();
                toActivityIterator.hasNext(); ) {
          ...
          if (((BranchCon) currentConnection).getTheDependency() != null) {
            Dependency newDependency = new Dependency();
            newDependency.setKindDep(
                ((BranchCon) currentConnection).getTheDependency().getKindDep());
          ...
          postProcessingCollection.add(currentConnection);
          this.addCoordinate(
              ((BranchCon) currentConnection).getId(),
              ((BranchCon) currentConnection).getClass().getSimpleName(),
              WebAPSEEObject.CONNECTION + "+" + newConnectionIdent,
              coordinates);
          ...
          postProcessingCollection.add(currentConnection);
          this.addCoordinate(
              ((JoinCon) currentConnection).getId(),
              ((JoinCon) currentConnection).getClass().getSimpleName(),
              WebAPSEEObject.CONNECTION + "+" + newConnectionIdent,
              coordinates);
    ...
    for (Iterator<Connection> postProcessingIterator = postProcessingCollection.iterator();
        postProcessingIterator.hasNext(); ) {
      ...
      if (currentPostConnection instanceof ArtifactCon) {
        ...
        for (Iterator<MultipleCon> fromMultipleConIterator = toMultipleCon.iterator();
            fromMultipleConIterator.hasNext(); ) {
        ...
        MultipleCon fromMultipleCon =
            ((BranchCon) currentPostConnection).getFromMultipleConnection();
        ...
        if (currentPostConnection instanceof BranchANDCon) {
          ...
          for (Iterator<MultipleCon> toMultipleConIterator = toMultipleCon.iterator();
              toMultipleConIterator.hasNext(); ) {
          ...
          Collection toMultipleCon =
              ((BranchConCond) currentPostConnection).getTheBranchConCondToMultipleCons();
          for (Iterator<BranchConCondToMultipleCon> toMultipleIterator = toMultipleCon.iterator();
              toMultipleIterator.hasNext(); ) {
            ...
            if (currentToMult != null) {
              ...
              if (currentToMult.getTheMultipleCon() != null) {
                ...
                if (newToMultipleCon != null) {
                  ((BranchConCondToMultipleCon) newBranchCondToMult)
                      .setTheMultipleCon(newToMultipleCon);
                } // end if
              } // end if != null
              // about conditions
              // newBranchConCondToMult.setCondition(currentToMult.getCondition());
              ((BranchConCond) alreadyCreatedConnection)
                  .addTheBranchConCondToMultipleCon(newBranchCondToMult);
            } // end if != null
          } // end for
        } // end if
  ...
  // 	// 	newToolParameters =
  // copySubroutineToolParameters(currentAutomatic.getTheSubroutine().getTheToolParameters(),
  // newSubRoutine);
  ...
  /** Taken from processUtility.CopyProcess */
  // private Set<ToolParameter> copySubroutineToolParameters(Collection theToolParameters,
  // Subroutine newSubRoutine) {
  // 	Set<ToolParameter> newParameters = new HashSet<ToolParameter>();

  // 	for (Iterator<ToolParameter> paramIterator = theToolParameters.iterator();
  ...
  /** Taken from processUtility.CopyProcess */
  // private Set<Parameter> copyAutomaticParameters(Collection theParameters, Automatic
  // newAutomatic) {
      ...
      if (ident.endsWith(" copy")) { // seq == 0
        ...
        if (lastToken.contains(" copy ")) {
          ...
          try {
            ...
            String identWithoutSeq =
                ident.substring(0, ident.length() - sequenceNumber.length() - 1);
            intSequenceNumber++;
            return this.getValidCopyId(
                identWithoutSeq + " " + intSequenceNumber, intSequenceNumber, properties);
  ...
  private Collection<Connection> getSimpleConnectionsToCopyFromActivity(
      Activity act, Collection sourceActivities) {
    ...
    while (iterConns.hasNext()) {
      ...
      if (connection != null) {
        if (connection instanceof SimpleCon) {
          ...
          if (sourceActivities.contains(simpleCon.getFromActivity())
              && sourceActivities.contains(simpleCon.getToActivity())) {
            copy.add(simpleCon);
          }
        }
      }
  ...
  /**
   * param predecessors is never null and never empty! return Object[] - ret[0] --> Date ret[1] -->
   * String
   */
   ...
   * 1. END-START
   *
   * <p>PlannedBegin(successor) = PlannedEnd(predecessor) + 1 day PlannedEnd(successor) =
   * PlannedBegin(successor) + How Long (business days)
   *
   * <p>2. END-END
   *
   * <p>PlannedBegin(successor) = PlannedEnd(sucessor) - HowLong (business days)
   * PlannedEnd(successor) = PlannedEnd(predecessor) + 1 day
   *
   * <p>3. START-START
   *
   * <p>PlannedBegin(successor) = PlannedBegin(predecessor) + 1 day PlannedEnd(successor) =
   * PlannedBegin(sucessor) + HowLong (business days)
   */
  // private void applyDatesFromPredecessor(Normal normal, Activity predecessor, String dependency,
  // Date greaterPredecessor,CalendarDTO replanningDates ) {
  ...
  // 				Date newPlannedEnd = new Date(this.plannedEnd(normal.getPlannedBegin().toEpochDay(),
  // oldHowLong));
  ...
  // 				int oldHowLong = Math.round(normal.getHowLong().floatValue());
  // 				Date newPlannedEnd = new Date(this.plannedEnd(normal.getPlannedBegin().getTime(),
  // oldHowLong));
  ...
  // 				Date newPlannedBegin = new Date(this.plannedBegin(normal.getPlannedEnd().getTime(),
  // Math.round(normal.getHowLong().floatValue())));
  // 				normal.setPlannedBegin(newPlannedBegin);
  ...
  // private boolean replanningNormalDates(Normal normal, Object[] greaterPredecessor,CalendarDTO
  // utilsDays) {
  ...
  // 		this.applyDatesFromPredecessor(normal, this.lastValid, (String) greaterPredecessor[1], (Date)
  // greaterPredecessor[0],utilsDays);

  // 		long newHowLong = normal.getHowLong().longValue();
  // 		if (normal.getPlannedBegin() != null && normal.getPlannedEnd() != null)
  // 			newHowLong = this.businessDays(normal.getPlannedBegin().toEpochDay(),
  // normal.getPlannedEnd().toEpochDay());
  ...
  // private void replanningDecomposedDates(Decomposed decomposed, Activity predecessor, Object[]
  // greaterPredecessor,CalendarDTO utilsDays) {

  // 	ProcessModel processModel = decomposed.getTheReferedProcessModel();
  // 	String state = processModel.getPmState();

  // 	if (!(state.equals(ProcessModel.FINISHED) || state.equals(ProcessModel.FAILED) ||
  // state.equals(ProcessModel.CANCELED))) { // It
  // 																																// means
  // 																																// that
  // 																																// decomposed
  // 																																// activity
  ...
  /** Returns a Collection with the successors (Activities only) of a Connection. */
  private Collection getSuccessors(Connection conn) {
    ...
    if (conn instanceof BranchCon) {
      ...
      if (branch instanceof BranchANDCon) {
        ...
        if (bAND.getToActivities() != null) succ.addAll(bAND.getToActivities());
        if (bAND.getToMultipleCons() != null) {
          ...
          while (iterMultis.hasNext()) {
            ...
            if (multi != null) succ.addAll(this.getSuccessors(multi));
          }
        }
      } else {
        ...
        while (iterAct.hasNext()) {
          ...
          if (act.getTheActivity() != null) succ.add(act.getTheActivity());
        }
      }
    } else if (conn instanceof JoinCon) {
      JoinCon join = (JoinCon) conn;
      if (join.getToActivity() != null) succ.add(join.getToActivity());
      if (join.getToMultipleCon() != null) succ.addAll(this.getSuccessors(join.getToMultipleCon()));
    }
    return succ;
  }

  /** Returns a Collection with the predecessors (Activities only) of a Connection. */
  private Collection getPredecessors(Connection conn) {
    ...
    if (conn instanceof BranchCon) {
      BranchCon branch = (BranchCon) conn;
      if (branch.getFromActivity() != null) pred.add(branch.getFromActivity());
      if (branch.getFromMultipleConnection() != null)
        pred.addAll(this.getPredecessors(branch.getFromMultipleConnection()));
    } else if (conn instanceof JoinCon) {
      JoinCon join = (JoinCon) conn;
      if (join.getFromActivities() != null) pred.addAll(join.getFromActivities());
  ...
  private Collection getConnectionsTo(Activity act) {
    ...
    if (act.getToSimpleCons() != null) connTo.addAll(act.getToSimpleCons());
    if (act.getToJoinCons() != null) connTo.addAll(act.getToJoinCons());
    if (act.getToBranchCons() != null) connTo.addAll(act.getToBranchCons());
    return connTo;
  }
  ...
  private Collection getConnectionsFrom(Activity act) {
    ...
    if (act.getFromSimpleCons() != null) connFrom.addAll(act.getFromSimpleCons());
    if (act.getFromJoinCons() != null) connFrom.addAll(act.getFromJoinCons());
    if (act.getFromBranchANDCons() != null) connFrom.addAll(act.getFromBranchANDCons());
    ...
    while (iterBctas.hasNext()) {
      ...
      if (bcta.getTheBranchConCond() != null) connFrom.add(bcta.getTheBranchConCond());
    }
    return connFrom;
  }
  ...
  // 	SimpleActivityQueryResult[] acts =
  // procRepository.getAllNormalActivitiesFromProcess(process_id);
  ...
  // 					if ((((ReqAgent) reqp).getTheRole() != null) && (((ReqAgent)
  // reqp).getTheRole().getIdent().equals(role_id))) {
