...
public class ProcessServicesImpl implements ProcessServices {
  ...
  // 		                     PROCESSAGENDA_CLASSNAME +" as procAg WHERE
  // procAg.theTaskAgenda.theAgent.ident =:agentId )";
  ...
  public ProcessesDTO getProjectsManagedBy(String agentIdent) {
    ...
    String hql_processes_with_manager =
        "select p from "
            + PROCESS_CLASSNAME
            + " as p left join p.theAgent ag "
  ...
  // 	String hql = "SELECT distinct proc FROM " + PROCESSAGENDA_CLASSNAME + " AS proc WHERE
  // proc.theTaskAgenda.theAgent.ident <> :ident";
  ...
  // 	// 	ProcessDTO processDTO = new ProcessDTO(processAgenda.getTheProcess().getIdent(),
  // processAgenda.getTheProcess().getpState(), this.getTasksFromProcess(processAgenda));
  ...
  private ActivityDTO convertActivityToActivityDTO(Activity activity) {
    ActivityDTO act =
        new ActivityDTO(activity.getName(), activity.getIdent(), "", 0.0, new Date(), new Date());
    return act;
  }
