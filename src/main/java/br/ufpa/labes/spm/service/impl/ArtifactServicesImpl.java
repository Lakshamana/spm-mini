...
public class ArtifactServicesImpl implements ArtifactServices {
  ...
  public ArtifactDTO saveArtifact(ArtifactDTO artifactDTO) {
    try {
      ...
      // artifactTypeRepository.retrieveBySecondaryKey(artifactDTO.getTheArtifactType());
    ...
    if (composicao) {
      hql =
          "select artifact from "
              + ARTIFACT_CLASS_NAME
              + " as artifact where artifact.belongsTo is null";
    } else {
      hql =
          "select artifact from "
              + ARTIFACT_CLASS_NAME
              + " as artifact where artifact.derivedFrom is null";
    }
    query = artifactRepository.getPersistenceContext().createQuery(hql);
    List<Artifact> artifacts = query.getResultList();
    return this.convertArtifactsToArtifactsDTO(artifacts);
  ...
  public ArtifactsDTO getArtifacts(
    ...
    if (domainFilter != null) {
      hql =
          "select art from "
              + ARTIFACT_CLASS_NAME
              + " as art where art.name like :termo and art.ident = :domain"
              + activeFilter;
      query = artifactRepository.getPersistenceContext().createQuery(hql);
      ...
      hql =
          "select art from "
              + ARTIFACT_CLASS_NAME
              + " as art where art.name like :termo"
              + activeFilter;
      query = artifactRepository.getPersistenceContext().createQuery(hql);
      query.setParameter("termo", "%" + termoBusca + "%");
    }
  ...
  private List<Artifact> usingProjectFilter(String projectFilter) {
    ...
    String hql =
        "SELECT project FROM "
            ...
            + " AS project WHERE project.name = :name";
  ...
  public ArtifactsDTO getArtifactsThatBelongsTo(String artifactName) {
    String hql =
        "select artifact from "
            + ARTIFACT_CLASS_NAME
            + " as artifact where artifact.belongsTo.name = '"
            + artifactName
            + "'";
    query = artifactRepository.getPersistenceContext().createQuery(hql);
    ...
    if (!resultado.isEmpty()) artifactsDTO = convertArtifactsToArtifactsDTO(resultado);
  ...
  public ArtifactsDTO getArtifactsDerivedFrom(String artifactName) {
    String hql =
        "select artifact from "
            + ARTIFACT_CLASS_NAME
            + " as artifact where artifact.derivedFrom.name = '"
            + artifactName
            + "'";
    query = artifactRepository.getPersistenceContext().createQuery(hql);
  ...
  public ArtifactDTO updateBelongsTo(String artifactName) {
    ...
    String hql =
        "select artifact from "
            + ARTIFACT_CLASS_NAME
            + " as artifact where artifact.name = '"
            + artifactName
            + "'";
    query = artifactRepository.getPersistenceContext().createQuery(hql);
    if (!query.getResultList().isEmpty()) {
  ...
  @Override
  public Map<String, SimpleArtifactDescriptorDTO[]> getArtifactsForSelectedActivity(
      String identActivity) {
    SimpleArtifactDescriptorDTO[] input =
        artifactRepository.getInputArtifactsForNormal(identActivity);
    SimpleArtifactDescriptorDTO[] output =
        artifactRepository.getOutputArtifactsForNormal(identActivity);

    Map<String, SimpleArtifactDescriptorDTO[]> result =
        new HashMap<String, SimpleArtifactDescriptorDTO[]>();
